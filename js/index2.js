$(document).ready(function(){
	...
	//getSales(localStorage['branch_id'],localStorage['company_id'],localStorage['terminal_id']);
			...
			if(t2 == 1){
				...
				if(memid == ''){
					...
					if(parseFloat($("#cashreceiveholder").text()) == 0){
						...
						printInvoiceOrDr(chkReceiptType,pageinvoice,pagedr,pageir);
						saveTransaction();
						//speedoff
						//refreshCartData();
	...
	$('body').on('keyup','.cartqty',function(){
		...
		if(isNaN(newqty)){
			...
			if (parenttr.attr('c-qty') == '-1' && parenttr.attr('c-days') == '-1'){
	...
	$('body').on('blur','.cartdiscount',function(){
		...
		if(isNaN(newdiscount) || newdiscount==''){
			...
			$(this).val(0);
			updatesubtotal();
			showToast('error','<p>Discount should be greater than or equal to 0</p>','<h3>WARNING!</h3>','toast-bottom-right');
			parenttr= $(this).parents('tr');
			qty =  parenttr.children().eq(0).find('input').val();
			price = parenttr.children().eq(2).text();
			newtotal = (parseFloat(price) * parseFloat(qty) -parseFloat($(this).val()));
	...
	function getItems(){
		if(localStorage["company_id"] != null && localStorage["branch_id"] != null && localStorage["terminal_id"] != null){
			// load items
			getProducts(localStorage["company_id"],localStorage["branch_id"],localStorage["terminal_id"],function(){
	...
	function getItemListSuggestion(){
		if(localStorage["items"] != null){
			...
			for(var item in items){
				$('#addproductincart').append("<option data-barcode='"+item+"'data-id='"+items[item].id+"'value='"+items[item].id+"'>"+item+": &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+items[item].item_code+":"+items[item].description+"</option>");
			}
			...
			function formatItem(o) {
				if (!o.id)
					...
					var r = o.text.split(':');
					return "<span> "+r[0]+"</span>" + r[1] + "<br><span style='margin-left:100px;' class='text-danger'><small class='testspanclass'>"+r[2]+"</small></span>";
				}
			...
			if(viewType == 1){
				...
				for(var i in items){
					...
					if(qty > 0 || qty == "<span class='text-success'>Available"){
						...
						$('.posview2').append('	<div data-qty="'+qty+'" data-barcode="'+i+'" class="col-sm-6 col-md-4 thumbItem">'+
							'<div class="thumbnail">' +
							'<img class="item_image" onError="this.onerror=null;this.src=\'item_images/no_image.png\';" src="item_images/'+item.id+'.jpg" alt="'+item.item_code+'"> ' +
							'<div class="caption"> ' +
							'<p>'+item.item_code+'</p>' +
							'<small  class="item_details">'+item.description+'</small>' +
							'<small class="item_details">'+number_format(item.price,2)+'</small>' +
							'</div>' +
							'</div>' +
							'</div>');
					}
	...
	function PrintElem(invoice_limit)
	{
		...
		$('#cart > tbody > tr').each(function(index){
			...
			lamankadainvoice[pagectr] = lamankadainvoice[pagectr] + "<tr ><td style='position:relative;width:"+styling['tdbarcode']['width']+"px;padding-left:"+styling['tdbarcode']['left']+"px;'>"+itemcode+"</td><td style='position:relative;width:"+styling['tdqty']['width']+"px;padding-left:"+styling['tdqty']['left']+"px;'>"+qty+"</td><td style='position:relative;width:"+styling['tddescription']['width']+"px;padding-left:"+styling['tddescription']['left']+"px;'> "+ description +" <span style='padding-left:20px;'>"+labeldisc+"</span> </td><td style='position:relative;width:"+styling['tdprice']['width']+"px;padding-left:"+styling['tdprice']['left']+"px;'>"+number_format(price,2)+"</td><td style='position:relative;width:"+styling['tdprice']['width']+"px;padding-left:"+styling['tdprice']['left']+"px;'>"+number_format(origtotal,2)+" "+labeldisc2+"</td></tr>";
	...
	function PrintElemIr(ir_limit)
	{
		...
		$('#cart > tbody > tr').each(function(index){
			...
			pagesubtotal = parseFloat(pagesubtotal) + parseFloat(total);
			lamankadadr[pagectr] = lamankadadr[pagectr] + "<tr ><td style='position:relative;width:"+styling['tdbarcode']['width']+"px;padding-left:"+styling['tdbarcode']['left']+"px;'>"+itemcode+"</td><td style='position:relative;width:"+styling['tdqty']['width']+"px;padding-left:"+styling['tdqty']['left']+"px;'>"+qty+"</td><td style='position:relative;width:"+styling['tddescription']['width']+"px;padding-left:"+styling['tddescription']['left']+"px;'> "+ description +" <span style='padding-left:20px;'>"+labeldisc+"</span> </td><td style='position:relative;width:"+styling['tdprice']['width']+"px;padding-left:"+styling['tdprice']['left']+"px;'>"+number_format(price,2)+"</td><td style='position:relative;width:"+styling['tdprice']['width']+"px;padding-left:"+styling['tdprice']['left']+"px;'>"+number_format(origtotal,2)+" "+labeldisc2+"</td></tr>";
			rowctr = parseInt(rowctr) +1;
		});
		if(pagesubtotal > 0){
			var subtotal = (pagesubtotal / vat);
