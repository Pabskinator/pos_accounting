<?php
	...
	if(!$user->hasPermission('sv_layout')){
		// redirect to denied page
		Redirect::to(1);
	}
	$barcodeClass = new Barcode();
	$barcode_format = $barcodeClass->get_sv_format($user->data()->company_id);

	$by_branch = $barcodeClass->get_format_by_branch($user->data()->branch_id,"SV");
	$has_own_layout = 0;
	if($by_branch){
		$has_own_layout = 1;
				...
				$.ajax({
					...
					success: function(data){
						alertify.alert(data,function(){
							location.href ='sv_generator.php';
						});
					},
					error:function(){
