...
?>
					...
					<div id="print-dr">
						...
						<div id='invalid-dr' style='display: none;'>
							<div class="alert alert-info">
								Invalid Request. Please set your computer as <strong>Terminal</strong> first.
							</div>
						</div>
					</div>
	...
	function fetchBatches(){
			$.ajax({
				...
				type:'POST',
				dataType:'json',
				data: {functionName:'getPendingBatch'},
				success: function(data){
					renderBatches(data)
				},
				error:function(){
		...
		function renderBatches(batches) {
			...
			if(batches.length == 0) {
				$('#list > h4')[0].innerHTML = 'No batch found';
			}
					...
					success: function(data){
						...
						for(var item in data) {
							...
							if(p == 0){
								...
								printAvisionDr(data[item],'Order Slip', dr_number++);
		...
		function renderPrintManifest() {
			...
			function isFinishedAddingSerials(){
				...
				for( var o in orders){
					...
					for( var i in items){
						if(items[i].qty != items[i].serials.length){
							answer = false;
							break;
						}
					}
			...
			function printAvisionDr(data, title, dr_number){
				...
				for(var p in pages){
					...
					for(var cp in cur_page){
						...
						paging += "<tr><td style='width:65%;'>"+description+"</td><td style='width:5%;'>"+qty+"</td><td style='width:15%;'>"+number_format(price,2)+"</td><td style='width:15%;'>"+number_format(total,2)+"</td></tr>";
					}
					if(ctr < item_per_page){
