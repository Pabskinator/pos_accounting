<?php
	class Cash  extends Crud implements PagingInterface{
		...
		public function getByPids($pids = ""){
			...
			if($pids){
				...
				$q= 'Select * from cash  where  payment_id in ('.$pids.') and is_active = 1';
		...
		public function countRecord($cid=0, $like='',$dt1=0,$dt2=0){
			...
			$q= "Select count(cc.id) as cnt from cash cc
		...
		public function get_active_record($cid,$start=0,$limit=0,$like='',$dt1=0,$dt2=0){
			...
			// prepare the query
			 $q= "Select cc.*,p.invoice, p.dr, p.ir , p.sold_date, m.lastname as member_name from cash cc
			left join	(Select * from sales group by payment_id )
			p on p.payment_id = cc.payment_id
			left join members m on m.id = p.member_id
			where p.company_id = ?  $likewhere $whereDate order by p.sold_date desc $l  ";
			//submit the query
			$data = $this->_db->query($q, $parameters);
